...
| **Pool worker** | Your GPU registers under someone else's orchestrator; they handle staking | Earning without managing LPT or on-chain setup |
| **Pool operator** | You run the node and stake; others contribute GPUs under your identity | Building a multi-node infrastructure business |

<Note>
  The fastest way to earn without touching LPT staking or on-chain activation is to **join an
  existing pool** as a worker. The rest of these docs assume you're running your own node; pool
  participation is operated through individual pool providers (Titan Node and others).
</Note>
